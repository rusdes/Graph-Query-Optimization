...
const expandShortForm = (shortForm) => {
  ...
  return (
    baseUri +
    (baseUri.endsWith("#") || baseUri.endsWith("/") ? "" : "#") +
    shortForm.split(":")[1]
  );
};
...
const prefixCreatedIfUnknownShortFormUsed = (input) => {
  ...
  let fullUri = prompt(
    "New prefix: which URI does " + shortForm + " stand for:",
    "http://onto.de/default/"
  );
  if (!fullUri) {
    return false;
  }
...
const getInput = (nodeOrEdge, isNode, callUpdates = true) => {
  let input = prompt(
    "Set a value for this " + (isNode ? "node" : "edge") + ":",
    (nodeOrEdge.isNewInConstruct ? "+" : "") + nodeOrEdge.label
  );
  ...
  if (!isNode && nodeOrEdge.source.type === "Literal") {
    alert(
      'This would turn the literal-node "' +
        nodeOrEdge.source.label +
        "\" into a subject, and subjects can't be literals"
    );
    return false;
  }
  ...
  graph = buildGraph(config)
    .onNodeDrag((dragNode) => {
      ...
      for (let node of nodes) {
        ...
        if (
          interimEdge &&
          node !== interimEdge.target &&
          distance(dragNode, node) < SNAP_IN_DISTANCE
        ) {
          removeEdge(interimEdge);
          setInterimEdge(dragSourceNode, node);
        }
      ...
      if (
        interimEdge &&
        distance(dragNode, interimEdge.target) > SNAP_OUT_DISTANCE
      ) {
    ...
    .nodeCanvasObject((node, ctx, globalScale) =>
      renderNode(node, ctx, globalScale, () => {
        return node === dragSourceNode ||
          (interimEdge &&
            (node === interimEdge.source || node === interimEdge.target))
          ? "IN_DRAGGING"
          : node.type;
      })
    )
    ...
    .linkCanvasObject(
      (edge, ctx, globalScale) =>
        edge !== interimEdge &&
        renderEdge(edge, ctx, globalScale, () => {
          return edge.type;
        })
    );
  update();
...
const graphChanged = () => {
  ...
  edges.forEach((edge) => {
    (edge.isNewInConstruct ||
    edge.source.isNewInConstruct ||
    edge.target.isNewInConstruct
      ? constructTriples
      : whereTriples
    ).push({
      subject: {
