...
public class LabelMatchingOperators {
	...
	public List<ArrayList<Long>> matchWithBounds(int col, int lb, int ub, String label, JoinHint strategy) throws Exception {
	...
	public List<ArrayList<Long>> matchWithLowerBound(int col, int lb, String label, JoinHint strategy) throws Exception {
	...
	private static class FilterEdgesByLabel implements FlatJoinFunction<Pair<Long, Long>, 
			EdgeExtended<Long, Long, String, HashMap<String, String>>, Pair<Long, Long>> {
		...
		public void join(
			...
			if(edge.f3.equals(lab))
				out.collect(new Pair<Long, Long>(vertexIds.f0, edge.f2));			
		}
	}
	
	private static class GetStartingVertexIds implements MapFunction<Pair<Long, Long>, 
			Pair<Long, Long>> {
		...
		public Pair<Long, Long> map(Pair<Long, Long> vertexIds)
			...
			return new Pair<Long, Long>(vertexIds.f0, vertexIds.f1);
		}
		
	}
	
	private static class DuplicatesReduction implements GroupReduceFunction<Pair<Long, Long>, 
			Pair<Long, Long>> {
		@Override
	...
	private static class GetNewResults implements CoGroupFunction<Pair<Long, Long>, 
